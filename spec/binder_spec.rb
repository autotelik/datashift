...
describe 'Binder Mapper' do
  ...
  let(:method_mapper)   { DataShift::Binder.new }
  ...
  it "should parse header for where  fields and make available through inbound_column" do
    ...
    headers.each_with_index do |c, i|
      expect(bindings[i].valid?).to eq true
      expect(bindings[i].inbound_column).to be_a DataShift::InboundColumn
      expect(bindings[i].inbound_column.lookup_list).to be_a Array
      expect(bindings[i].inbound_column.lookup_list.size).to eq 1
      expect(bindings[i].inbound_column.lookup_list[0]).to be_a DataShift::LookupSupport
      expect(bindings[i].inbound_column.first_lookup).to be_a DataShift::LookupSupport
  ...
  it "should parse header for where clause field" do
    ...
    headers.each_with_index do |c, i|
      ...
      expect(first_lookup.value).to eq nil
    end
  end
  ...
  it "should parse header for where clause field & value belongs_to" do
    ...
    headers.each_with_index do |c, i|
      ...
      expect(bindings[i].inbound_column.first_lookup.value).to eq 'my first project'
    end
  end
  ...
  it "should parse header for where clause field & value has_one" do
    ...
    headers.each_with_index do |c, i|
      ...
      expect(bindings[i].inbound_column.first_lookup.value).to eq '10.2'
    end
  end
  ...
  it "should parse header for where clause field & value & global data has_many" do
    ...
    headers.each_with_index do |c, i|
      ...
      expect(bindings[i].inbound_column.first_lookup.value).to eq 'my pdf'
  ...
  it "should raise an error when association has no such field" do
    ...
    expect { method_mapper.map_inbound_headers( Owner, headers ) }.to raise_error DataShift::NoSuchOperator
  end
