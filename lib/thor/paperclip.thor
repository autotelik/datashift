...
module Datashift
  ...
  class Paperclip < Thor     
    ...
    method_option :attach_to_field, :required => true, :aliases => '-f', :desc => "The field to use to find :attach_to_klass record"
    
    method_option :split_file_name_on,  :type => :string, :desc => "delimiter to progressivley split filename for lookup", :default => '_'
    ...
    def attach()
      ...
      loader = DataShift::Paperclip::AttachmentLoader.new(klazz, nil, options.dup)
   
      logger.info "Loading attachments from #{@attachment_path}"

      loader.process_from_filesystem(@attachment_path, options)
